...
function setupTargetCanvas(target) {
    var canvas = target.getElementsByTagName('canvas');
    if (canvas) {
        canvas = canvas[0];
    }
    else {
        canvas = document.createElement('canvas');
        canvas.setAttribute('id', target.id.replace('target-box', 'canvas'));
...
function revealTargetItem() {
    targetBox = menuBox;
    var items = targetBox.getElementsByClassName('speaker-only');
    for (var i = 0; i < items.length; i++) {
        items[i].classList.remove('speaker-only');
    }
